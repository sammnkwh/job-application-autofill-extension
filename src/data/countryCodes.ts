// Country code data with emoji flags for phone number input
...
// Full list of all countries sorted alphabetically
...
export function findCountryByCode(code: string): CountryCode | undefined {
  return ALL_COUNTRIES.find(c => c.code === code) || COMMON_COUNTRIES.find(c => c.code === code)
}

/**
 * Find a country by its dial code (returns first match, preferring US for +1)
 */
export function findCountryByDialCode(dialCode: string): CountryCode | undefined {
  // For +1, prefer US over Canada
  if (dialCode === '+1') {
    return ALL_COUNTRIES.find(c => c.code === 'US')
  }
...
export function parsePhoneWithCountryCode(phone: string): { countryCode: string; phoneNumber: string } {
  ...
  // Check if phone starts with a + (has country code)
  if (trimmed.startsWith('+')) {
    // Try to match country codes from longest to shortest (e.g., +1684 before +1)
    const sortedCountries = [...ALL_COUNTRIES].sort((a, b) => b.dialCode.length - a.dialCode.length)

    for (const country of sortedCountries) {
      if (trimmed.startsWith(country.dialCode)) {
        const phoneNumber = trimmed.slice(country.dialCode.length).trim()
